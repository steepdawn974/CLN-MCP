...
impl NodeService {
    ...
    #[tool(
        description = "listaddresses - List all addresses of the node (both announced and binding addresses)"
    )]
    ...
    #[tool(
        description = "listchannels - Query active lightning channels in the entire network. Optionally filter by short_channel_id, source, or destination"
    )]
    ...
    #[tool(
        description = "listpeerchannels - List channels with directly connected peers, showing their current state"
    )]
    ...
    #[tool(
        description = "listclosedchannels - List channels that have been closed, with details on close cause and fees"
    )]
    ...
    #[tool(
        description = "listhtlcs - List all HTLCs (Hash Time Locked Contracts) with their current state"
    )]
    ...
    #[tool(
        description = "listpays - List payment results, optionally filtered by bolt11 invoice string"
    )]
    ...
    #[tool(
        description = "listsendpays - List outgoing payments with more detail than listpays, optionally filtered by bolt11"
    )]
    ...
    #[tool(
        description = "listforwards - List all HTLCs that have been forwarded by the node, optionally filtered by status, in_channel, or out_channel"
    )]
    ...
    #[tool(
        description = "listinvoices - Query invoice status. Optionally filter by label, invstring, payment_hash, or offer_id"
    )]
    ...
    #[tool(
        description = "listpeers - List connected lightning nodes. Optionally filter by id or show log level"
    )]
    ...
    #[tool(
        description = "bkpr-channelsapy - List APY stats for channels, optionally filtered by account"
    )]
    ...
    #[tool(
        description = "bkpr-listincome - List income events (routing fees, on-chain transactions, etc.)"
    )]
    ...
    #[tool(
        description = "bkpr-listaccountevents - List detailed accounting events (channel opens, closes, HTLCs, etc.)"
    )]
    ...
    #[tool(
        description = "getroute - Find the best route for a payment. Requires id (pubkey) and amount_msat. Optional riskfactor"
    )]
    ...
    #[tool(description = "decodepay - Decode a bolt11 invoice string and show its details")]
    pub async fn decode_pay(
        &self,
        Parameters(params): Parameters<DecodePayParams>,
    ) -> Result<CallToolResult, ErrorData> {
        self.call_backend("decodepay", serialize_params(&params))
            .await
    }
    ...
    #[tool(
        description = "listtransactions - List on-chain transactions from the wallet. Optional start and limit for pagination"
    )]
    pub async fn list_transactions(
        &self,
        Parameters(params): Parameters<ListTransactionsParams>,
    ) -> Result<CallToolResult, ErrorData> {
        self.call_backend("listtransactions", serialize_params(&params))
            .await
    }
    ...
    #[tool(
        description = "fundchannel - Open a channel with a connected peer. Requires id (pubkey) and amount (sat). Optional feerate and announce"
    )]
